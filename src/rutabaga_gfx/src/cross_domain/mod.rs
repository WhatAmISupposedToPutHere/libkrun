...
impl CrossDomainFutex {
    fn watcher_thread(address: FutexPtr, shutdown: Arc<AtomicBool>, sender: Sender) {
        ...
        let atomic_val = unsafe {
            AtomicU32::from_ptr(address as *mut u32)
        };
        loop {
...
impl CrossDomainContext {
    ...
    fn futex_new(&mut self, cmd_futex_new: &CrossDomainFutexNew) -> RutabagaResult<()> {
        ...
        let path = PathBuf::from(format!(
            "/dev/shm/shm-{}", String::from_utf8_lossy(&cmd_futex_new.filename.to_ne_bytes())
        ));
        ...
        let address = unsafe {
            mmap(None, 4.try_into().unwrap(), ProtFlags::PROT_WRITE | ProtFlags::PROT_READ, MapFlags::MAP_SHARED, handle.as_raw_fd(), 0)?
        };
        unlink(&path)?;
        ...
        futexes.insert(cmd_futex_new.id, CrossDomainFutex {
            handle, address, watcher_thread, shutdown
        });
        ...
        while !commands.is_empty() {
            ...
            match hdr.cmd {
                ...
                CROSS_DOMAIN_CMD_FUTEX_SIGNAL => {
                    let cmd_futex_signal = CrossDomainFutexSignal::read_from_prefix(commands.as_bytes())
                        .ok_or(RutabagaError::InvalidCommandBuffer)?;
                    self.futex_signal(&cmd_futex_signal)?;
                }
                CROSS_DOMAIN_CMD_FUTEX_DESTROY => {
                    let cmd_futex_destroy = CrossDomainFutexDestroy::read_from_prefix(commands.as_bytes())
                        .ok_or(RutabagaError::InvalidCommandBuffer)?;
